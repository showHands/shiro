...
CREATE TABLE `tbUser` (
  ...
  `nLock` int(1) DEFAULT '1' COMMENT '锁定状态：0-未锁 1-锁定',
...
CREATE TABLE `tbUserRole` (
  ...
  `strUserName` varchar(60) NOT NULL DEFAULT '' COMMENT '用户名称',
  `strRole` varchar(60) NOT NULL DEFAULT '' COMMENT '角色名称',
  `strDescription` varchar(30) NOT NULL DEFAULT '' COMMENT '用户角色描述',
  `nState` int(1) DEFAULT '1' COMMENT '用户角色状态：1-可用  2-不可用',
  `dtCreateTime` datetime NOT NULL COMMENT '创建时间',
  `dtModifyTime` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '修改时间',
  PRIMARY KEY (`lId`),
